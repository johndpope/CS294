...
def main():
    ...
    parser.add_argument('--n_opt_steps', type=int, default=3,
    ...
    with tf.Session():
        ...
        for i in range(args.num_rollouts):
            ...
            if (i % args.expert_interval == 0):
                obs_2nrm, obs_dnrm = dutl.normalize(observations)
                acn_2nrm, acn_dnrm = dutl.normalize(expert_actions)
                for j in range(args.n_opt_steps):
                    _, loss_val_train = step_fn(
                        obs_2nrm(observations[-args.max_timesteps:]),
                        acn_2nrm(expert_actions[-args.max_timesteps:]))
            # loss_total = None
            # if not loss_total:
            #     loss_total = loss_val
            # else:
            ...
            if not (i % args.expert_interval == 0):
