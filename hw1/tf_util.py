...
def l2loss(params):
    if len(params) == 0:
        ...
        return tf.add_n([tsum(tf.square(p)) for p in params])
...
def wndense(x, size, name, init_scale=1.0):
    ...
    x = tf.matmul(x, v)
    scaler = g / tf.sqrt(tsum(tf.square(v), axis=0, keepdims=True))
...
def batchnorm(x, name, phase, updates, gamma=0.96):
    ...
    mean_ = tmean(x, axis=0, keepdims=True)
    var_ = tmean(tf.square(x), axis=0, keepdims=True)
    std = tf.sqrt(var_)
    trainy = (x - mean_) / std
...
class _MemFriendlyFunction(object):
    def __init__(self, nondata_inputs, data_inputs, outputs, batch_size):
        """
        IK:
        nondata_inputs: placeholders for non data inputs
        data_inputs: placeholders for data inputs
        outputs: result of a model operating on inputs
        """
    ...
    def __call__(self, *inputvals):
        # inputvals: concatenated list of values for nondata and data
        ...
        for i_start in range(0, n, self.batch_size):
            ...
            # list of data_vals' current batch slices
            for (var,val) in zip(self.data_inputs, slice_vals):
                feed_dict[var]=val
            results = tf.get_default_session().run(self.outputs, feed_dict=feed_dict)  # evaluate model over current feed_dict
            if i_start==0:
                sum_results = np.array(results,dtype=np.float32)
            else:
                sum_results += np.array(results,dtype=np.float32)
...
def module(name):
    """
    IK:
    Creates new module with module named 'name'.
    Returns a wrapper function which calls function f, passed as input, and
    runs it in Module with name 'name'
    """
    @functools.wraps
    def wrapper(f):
