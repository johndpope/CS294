...
def model3(sy_ob_no, ac_dim):
    ...
    sy_h1 = tf.nn.relu(dense(sy_ob_no, 128, "h1", weight_init=w_init))\
        # hidden layer
    sy_h2 = tf.nn.relu(dense(sy_h1, 64, "h2", weight_init=w_init))
    b_init=tf.random_normal_initializer(stddev=.5)
    sy_mean_na = dense(sy_h2, ac_dim, "mean", weight_init=w_init, bias_init=b_init)\
        # mean control output
    b_init=tf.random_normal_initializer(mean=.5, stddev=.01)
    sy_std_na = tf.nn.softplus(dense(sy_h2, ac_dim, "std",weight_init=w_init,bias_init=b_init))\
        # parametrized stddev of control output
    return tf.squeeze(sy_mean_na), tf.squeeze(sy_std_na)
...
class Summarizer(object):
    def __init__(self, sess, summaries_dir='summaries/',name='train'):
        self.merged = tf.summary.merge_all()
        self.train_writer = tf.summary.FileWriter(summaries_dir + name, sess.graph)
        self.sess = sess
    def create_summary(self, ix):
...
def main_pendulum(n_iter=int(1e6), gamma=.95, min_timesteps_per_batch=1000,
    ...
    for i in range(n_iter):
        ...
        summary.create_summary(total_timesteps)
