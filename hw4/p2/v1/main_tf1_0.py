...
class NonLinearValueFunction(object):
    ...
    def set_net_vars(self, _sess, sy_X, sy_y, optimizer=None, stepsize=1e-3, reg_coeff=1e-3):
        self.sess = _sess
        self.X = sy_X
        self.y = sy_y
        ...
        self.update = optimizer(stepsize).minimize(self.loss+reg_coeff*self.reg)
    def predict(self,X):
...
def model2(sy_ob_no, ac_dim):
    ...
    sy_std_na = tf.nn.softplus(dense(sy_h1, ac_dim, "std",
                                     weight_init=tf.random_normal_initializer(mean=1, stddev=.5)))\
        # parametrized stddev of control output
    return tf.squeeze(sy_mean_na), tf.squeeze(sy_std_na)
...
def model3(sy_ob_no, ac_dim):
    ...
    sy_std_na = tf.nn.softplus(dense(sy_h2, ac_dim, "std",
                                     weight_init=tf.random_normal_initializer(mean=1.4,
                                                                              stddev=.2)))\
        # parametrized stddev of control output
    return tf.squeeze(sy_mean_na), tf.squeeze(sy_std_na)
...
class NonLinearPolicyFunction(object):
    def loss_fn(self, reg_coeff):
        loss = -tf.reduce_mean(self.sy_adv_n * self.sy_logprob_n)\
            # Loss function that we'll differentiate to get the policy gradient ("surr" is for\
            # "surrogate loss")
        t_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="policy_function")
        reg = sum(tf.reduce_mean(tf.abs(t)) for t in t_vars)  # regularization
        return loss+reg_coeff*reg
    def __init__(self, sess, sy_ob_no, policy_model, sampler, sy_y_old, sy_ac_n, dist_and_logprobfn,
                 calc_kl, calc_ent, sy_adv_n, ac_dim, sy_stepsize, optimizer=None, reg_coeff=1e-3,
                 n_steps=1):
        self.sess = sess
        ...
        self.sy_sampled_ac = sampler(self.sy_y_pred,self.sy_n)
        self.dist, log_prob_fn = dist_and_logprobfn(self.sy_y_pred)
        self.sy_y_old = sy_y_old
        self.old_dist, _ = dist_and_logprobfn(self.sy_y_old)
...
def main_pendulum(n_iter=int(1e6), gamma=.95, min_timesteps_per_batch=1000,
    ...
    if ac_dim == 1:
        ...
        def sampler(y, sy_n):
            sy_mean, sy_std = y
            return random_ops.parameterized_truncated_normal(shape=[sy_n],
                                                             means=sy_mean,
                                                             stddevs=sy_std,
    ...
    def calc_kl(old_dist,dist):
        return tf.reduce_mean(ds.kl(old_dist,dist))
    ...
    sess = tf.Session()
    sess.__enter__() # equivalent to `with sess:`
    pf = NonLinearPolicyFunction(sess, sy_ob_no, model3, sampler,
                                 (sy_oldmean_na, sy_oldstd_na), sy_ac_n, dist_and_logprobfn,
                                 calc_kl, calc_ent, sy_adv_n, ac_dim, sy_stepsize)
    ...
    for i in range(n_iter):
        ...
        evbefore = explained_variance_1d(vpred_n, vtarg_n)
        evafter = explained_variance_1d(vf.predict(ob_no), vtarg_n)
        pltu.plot2(total_timesteps,eprewmean)
        pltu.plot1(total_timesteps,evbefore,0)
        pltu.plot1(total_timesteps,ent,1)
        logz.log_tabular("EpRewMean", eprewmean)
        logz.log_tabular("EpLenMean", np.mean([pathlength(path) for path in paths]))
