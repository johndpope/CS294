...
class Summarizer(object):
    def __init__(self, sess, sy_X, summaries_dir='../../summaries/p2/v5/', name='r8'):
...
def main_pendulum(n_iter=int(1e6), gamma=.95, min_timesteps_per_batch=1000,
    ...
    sess = tf.Session()
    sess.__enter__() # equivalent to `with sess:`
    pf = NonLinearPolicyFunction(sess, sy_ob_no, model2, (sy_oldmean_na, sy_oldstd_na), sy_ac_n,
    ...
    summary = Summarizer(sess, [sy_ob_no, sy_ac_n, sy_adv_n, (sy_oldmean_na, sy_oldstd_na),
                                sy_reward, sy_evbefore, sy_evafter])
